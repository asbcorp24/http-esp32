...
static void saveConfig() {
  ...
  prefs.putUShort("sampleInt", clampInterval(cfg.sampleIntervalSec));
  prefs.putUChar("phasePct", clampPhasePct(cfg.phaseImbalancePct));
}
...
static String htmlPage() {
  ...
  h += ".wrap{max-width:760px;margin:0 auto;padding:16px;}";
  ...
  h += "h2{font-size:16px;margin:18px 0 10px;}";
  h += "label{display:block;margin:10px 0 6px;font-size:14px;opacity:.95;}";
  h += "input,textarea{width:100%;padding:12px;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:#0b1220;color:#e8eefc;outline:none;box-sizing:border-box;}";
  ...
  h += "<div class='row'>";
  h += "<div><label>Интервал опроса и отправки, сек</label>";
  h += "<input name='sampleIntervalSec' type='number' min='15' max='60' required value='" + String(cfg.sampleIntervalSec) + "'/></div>";
  h += "<div><label>Допустимый перекос фаз, %</label>";
  h += "<input name='phaseImbalancePct' type='number' min='0' max='20' required value='" + String(cfg.phaseImbalancePct) + "'/></div>";
  h += "</div>";
  ...
  h += "<input name='voltage' type='number' step='0.1' value='" + String(cfg.voltage, 1) + "'/>";
  ...
  h += "<div class='muted'>Wi-Fi конфиг включается только при замыкании GPIO4 на GND при старте.<br>";
  ...
  h += "if(r.ok){const ok=document.getElementById('ok');ok.style.display='block';setTimeout(()=>ok.style.display='none',1800);}else alert('Ошибка сохранения');";
  h += "});";
  h += "</script>";
...
static void webSetupRoutes() {
  ...
  web.on("/save", HTTP_POST, []() {
    ...
    if (!cfg.serverHost.length()) cfg.serverHost = "specdpo.ru";
    cfg.sampleIntervalSec = clampInterval(cfg.sampleIntervalSec);
    cfg.phaseImbalancePct = clampPhasePct(cfg.phaseImbalancePct);
...
void WifiConfigStart(const Config& initialCfg) {
  ...
  if (!prefs.isKey("voltage")) cfg.voltage = initialCfg.voltage;
  if (!prefs.isKey("sampleInt")) cfg.sampleIntervalSec = initialCfg.sampleIntervalSec;
  if (!prefs.isKey("phasePct")) cfg.phaseImbalancePct = initialCfg.phaseImbalancePct;
